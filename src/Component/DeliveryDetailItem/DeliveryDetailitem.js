...
const DeliveryDetailitem = (props) => {
  ...
  const totalForEachItem = count * price;

  useEffect(() => {
    //Find index of specific object using findIndex method and updating it's count value.
    const objIndex = foodCart.findIndex((obj) => obj.id == id);
    const newArray = [...foodCart];
    newArray[objIndex].count = finalCount;
    setFoodCart(newArray);
    // console.log("After update: ", foodCart);
  },[finalCount])
    ...
    <div style={style.div1}>
      ...
      <div style={style.div3}>
        <p style={style.p} onClick={() => props.handleRemove(id)}>
          <span style={style.span}>X</span> Remove
        </p>
